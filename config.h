...
static const char *notescmd[]    = { "chinotes", NULL };
static const char *notesaddcmd[] = { "chinotes", "add", NULL };
static const char *manchmd[]    = { "chiman", NULL };
static const char *dispcmd[]    = { "chidisp", NULL };
static const char *overviewcmd[]= { "chioverview", NULL };
static const char *layoutcmd[]  = { "chilayout", NULL };
static const char *batttimecmd[]= { "dwmstatus", "batttime-toggle", NULL };
static const char *scrotcmd[]    = { "gnome-screenshot", NULL };
static const char *scrotregion[] = { "gnome-screenshot", "-a", NULL };
/* XAUTHORITY must be passed explicitly — doas strips the environment */
...
static const Key keys[] = {
	...
	{ MODKEY,                       XK_n,            spawn,          {.v = notescmd } },  /* Super+N         → view notes */
	{ MODKEY|ShiftMask,             XK_n,            spawn,          {.v = notesaddcmd} },/* Super+Shift+N   → add note */
	{ MODKEY|ShiftMask,             XK_m,            spawn,          {.v = manchmd } },   /* Super+Shift+M   → man page picker */
	{ MODKEY|ShiftMask,             XK_d,            spawn,          {.v = dispcmd } },   /* Super+Shift+D   → display config */
	{ MODKEY,                       XK_Tab,          spawn,          {.v = overviewcmd} },/* Super+Tab       → window overview */
...
/*	{ MODKEY, XK_F1,                     spawn, {.v = volup } },      test: Super+F1 → vol up */
	...
	{ MODKEY|ShiftMask,             XK_F4,           restart,        {0} },               /* Super+Shift+F4  → restart dwm (keep windows) */
	{ MODKEY|ShiftMask,             XK_b,            spawn,          {.v = batttimecmd} },/* Super+Shift+B   → toggle battery time */
	{ MODKEY|ShiftMask,             XK_l,            spawn,          SHCMD("doas env XAUTHORITY=\"$XAUTHORITY\" chilock") }, /* Super+Shift+L → lock */
	{ MODKEY|ShiftMask,             XK_x,            spawn,          SHCMD("chiproc") },  /* Super+Shift+X   → kill process */
	{ MODKEY,                       XK_semicolon,    spawn,          SHCMD("chirun") },   /* Super+;         → run command in st */
